...
\caption{Spectre des technologies sans fil (source : \ref{lpwan1}}\label{term101}
\end{figure}
...
LoRa est principalement utilisée dans l'IoT. Elle se distingue par sa portée étendue, qui peut atteindre plusieurs kilomètres en milieu urbain et plusieurs dizaines de kilomètres en milieu rural, ainsi que par sa faible consommation d'énergie, qui permet de prolonger la durée de vie des appareils connectés. Lora fait partie des technologies appelées \textit{LPWAN (Low Power Wide
 Area Network)}. La figure \ref{term101} montre le spcetre des technologies sans fil actuelles. Une longue portée avec un puissance limitée induit un débit de transmission plus faible que les autres technologies sans fil (le Wifi, la 4G, Bluetooth etc).
...
\item Le \textit{Gray mapping}. Le code de Gray est une méthode d'ordonnancement de symboles. Le principe de cette méthode est que dans la séquence ordonnée, chaque symbole ne diffère du précedent que d'un seul bit. Utiliser le code de Gray permets de réduire les erreurs dues au bruit ou aux interférences.
...
La modulation LoRa repose sur le principe de \textit{Frequency Shift Chirp Modulation (FSCM)}, qui est une combinaison de la modulation FSK et CSS. 
La modulation CSS étale le signal sur une large bande de fréquence tandis que la modulation FSK shift la fréquence périodiquement dans le temps. Le signal modulé est composé de chrip. Un chirp est un signal dont la fréquence change en continue tout en conservant une amlitude constante. Il existe deux types de chirps : les $upchirp$ et $downchirp$.
...
Supposons que la band passante du canal soit $\beta$, LoRa impose qu'un échantillons soit transmis tout les $T$ = $\frac{1}{\beta}$. Un symbole $s(nT_s)$ est envoyé a l'entré de la modulation tout les $T_s$ = $2^{SF}$ où \textit{SF (spreading factor)} est le facteur d'étalement. le symbole $s(nT_s$) est un nombre réel formé en utilisant un vecteur de chiffre binaire du facteur d'étalement. Les valeurs pour LoRa sont comprises entre 7 et 12. Lora utilise donc au total $2^{SF}-1$ symboles. L'onde transmise pour une durée $T_s$ pour un certain $s(nT_s)$ vaut :
