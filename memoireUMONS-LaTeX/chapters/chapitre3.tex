...
La plupart des commentaires sur la première table sont valables pour la table \ref{signature2}. Cependant, on constate qu'un spreading factor maximal (SF = 12) génère une seconde signature. On remarque également que la signature se trouve à l'extérieur du cercle.
...
Même si dans un premier temps les signatures ont des formes similaires, s'apparentant à des comètes, il y a cepedant des différentes notable entre chaque module. En effet, les signatures du premier module ont une tête de comète arrondie, là où celle du second module ont une finition pointue. La queue de la comète des signatures du second module est systématiquement détachée du reste du corps. La tête de la comète des signatures du troisème module est plus grosse que les deux autres, avec un corps plus étendu.